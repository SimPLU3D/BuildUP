...
configuration* bldg_generator(Param& p, Lot* lot,std::ofstream& ofs_energy,std::ofstream& ofs_geom,int& iter)
{
   ...
   /*** configuration ***/
    ...
    VariateAll vAll(vC,vRo,vT,vRa,vH,predicate);
    Birth birth(vAll);
    ...
    simulated_annealing::max_iteration_end_test endtest(p.get<int>("nbiter"));
    ...
    simulated_annealing::optimize(engine,*conf,samp,schedule,endtest,visitor);
    return conf;

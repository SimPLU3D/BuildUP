...
public:
    ...
    virtual ~EnergyComposite(){if(_leftChild!=NULL) delete _leftChild;if(_rightChild!=NULL) delete _rightChild;}
...
public:
    ...
    ~EnergyPiecewise(){if(!_energyMap.empty()) for(EnergyMap::iterator it=_energyMap.begin(); it!= _energyMap.end();++it) delete it->second;}

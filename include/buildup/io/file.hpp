...
void load_bldgsFinal_shp(const char* file,std::vector<Building> & bldgs)
{
    bldgs.clear();
    OGRRegisterAll();
    OGRDataSource *poDS = OGRSFDriverRegistrar::Open(file,FALSE);
    ...
    while( (poFeature = poLayer->GetNextFeature()) )
    {
        ...
        bldgs.push_back(Building(ply,h,lotID));
...
//    void save_lots(std::map<int,Lot>& lots,const char* file)

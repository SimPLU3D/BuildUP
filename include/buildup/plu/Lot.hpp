...
public:
    ...
    BorderSeg(int id_lot,int id,Segment_2& seg,BorderType t=BorderType::Unknown,int id_border=-1): _id_lot(id_lot),_id(id),_seg(seg),_type(t),_id_border(id_border) {}

    inline int getID()
    {
        return _id;
    }
    inline Segment_2& getGeom()
    {
        return _seg;
    }
    inline BorderType getType()
    {
        return _type;
    }
    inline int getBorderID()
    {
        return _id_border;
    }

    inline void setGeom(Segment_2& seg)
    {
        _seg=seg;
    }
    inline void setType(BorderType t)
    {
        _type=t;
    }
    inline void setBorderID(int i)
    {
        _id_border=i;
    }
...
public:
    ...
    Border(int id_lot,int id,BorderType type,const char* name):_id_lot(id_lot),_id(id),_type(type),_typeName(name) {}

    inline void addSeg(BorderSeg* pSeg)
    {
        _pSegs.push_back(pSeg);
    }

    inline std::vector<BorderSeg*>& getSegs()
    {
        return _pSegs;
    }
    inline Segment_2& getSeg(int i)
    {
        return _pSegs.at(i)->getGeom();
    }
    inline int getID()
    {
        return _id;
...
public:
    ...
    Lot(int id, OGRPolygon* ply): _id(id),_polygon((OGRPolygon*)(ply->clone())),_area(ply->get_Area())
        ,_idRefSeg(-1),_thetaRefSeg(-1),_translatedX(0.),_translatedY(0.)
    {
    ...
    inline OGRPolygon* polygon() const
    {
        return _polygon;
    }
    ...
    inline RuleEnergy* ruleEnergy(RuleType t)
    {
        return _ruleEnergy[t];
    }
    inline RuleGeom* ruleGeom() const
    {
        return _ruleGeom;
    }
    inline double translatedX() const
    {
        return _translatedX;
    }
    inline double translatedY() const
    {
        return _translatedY;
    }
    inline double invTransX() const
    {
        return -_translatedX;
    }
    inline double invTransY() const
    {
        return -_translatedY;
    }

    //sets
    inline void insert_borderSeg(int id,BorderSeg& seg)
    {
        _borderSegs.insert(std::make_pair(id,seg));
    }
    inline void insert_border(int id,Border& border)
    {
        _borders.insert(std::make_pair(id,border));
    }
    inline void add_seg2border(int idSeg, int idBorder)
    {
        (_borders.find(idBorder)->second).addSeg(&(_borderSegs.find(idSeg)->second));
    }
    ...
    inline void insert_ruleEnergy(RuleType t, RuleEnergy* r)
    {
        _ruleEnergy[t] = r;
    }
    inline void set_ruleGeom(RuleGeom* r)
    {
        _ruleGeom=r;
    }
    ...
    inline bool hasBorder(int id) const
    {
        return _borders.find(id)!=_borders.end();
    }
    inline bool hasRule(RuleType rule)
    {
        return _ruleEnergy[rule]!=0;
    }

    template<typename Iterator>
    double getRefTheta(Iterator it)
    {
        //Theta of its nearest border segment
        if(_idRefSeg>-1)
            return _thetaRefSeg;

        Point_2 center(*it,*(it+1));
    ...
    template<typename Iterator>
    double getRefTheta_front(Iterator it)
    {
        if(_idRefSeg>-1)
            return _thetaRefSeg;

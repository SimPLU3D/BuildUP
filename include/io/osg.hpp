...
namespace io
{
    ...
    void display(std::vector<Bldg>& bldgs, std::map<int,Lot>& lots)
    {
        ...
        {
            OGRPolygon *ply = (OGRPolygon*)(it->second._polygon->Buffer(-0.1));
            root->addChild(makeGeode(ply,osg::PrimitiveSet::Mode::POLYGON,true,WHITE));
            ply->empty();
            root->addChild(makeGeode(it->second._polygon,osg::PrimitiveSet::Mode::LINE_LOOP,false,BLACK));
        }
...
//                    exp->addChild(label);
...

        {
            ...
            for(it=lots.begin();it!=lots.end();++it)
            {
                ...
                ply->empty();
                gpLots->addChild(makeGeode(it->second._polygon,osg::PrimitiveSet::Mode::LINE_LOOP,false,BLACK));
            }
